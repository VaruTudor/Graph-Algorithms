...
#######################################################################################################################################
    ...
    def lowest_cost_path(self, ver1, ver2):
        shorthest_path = list() # holds the path
        shorthest_path.append(ver1)
        p_matrices = list() # holds the previus matrices
        m = GMatrix(self.n)
        p_matrices.append(m.path_m)
        for e in self.D_cost:
            # the matrix is initialized with the graph
            v1 = e[0]
            v2 = e[1]
            cost = self.D_cost[e]
            m.add_element(v1, v2, cost)
        ...
        for i in range(self.n):
            # here we check for negative cycles
            if m.matrix[i][i] < 0:
                raise NegativeCycle

        for i in reversed(p_matrices):
            # vertices are added to the shortest path list
            if i[ver1][ver2] != 0 and i[ver1][ver2] not in shorthest_path:
                shorthest_path.append(i[ver1][ver2])
        
        shorthest_path.append(ver2)

        return (m.matrix[ver1][ver2],shorthest_path)        
