...
class UndirectedGraph:
    ...
    def read_file(self, file_name):
        ...
        while True:
            ...
            self.D_edges.append( (v1, v2) )
            self.D_edges.append( (v2, v1) )
    ...
    def addEdge(self, x, y):
        ...
        self.D_edges.append( (x,y) )
        self.D_edges.append( (y,x) )
        self.m += 1
    ...
    def vertex_cover(self):
        # the minimum vertices which cover the graph
        vertices_covering = set()
        # holds all the edges
        edges = set()
        # holds all the degrees of each vertex in order
        vertex_degrees = dict()
        for v in self.D_edges:
            edges.add(v)
        for v in list(self.D_vertices):
            vertex_degrees[v] = len(self.D_vertices[v]) 
        
        while( len(edges) > 0 ):
            max_degree_vertex = max(vertex_degrees, key=vertex_degrees.get)
            vertices_covering.add(max_degree_vertex)
            for v in self.D_vertices[max_degree_vertex]:
                try:
            ...
            del vertex_degrees[max_degree_vertex]
            
        return vertices_covering


    def vertex_cover_aproximation(self):
        # the minimum vertices which cover the graph
        vertices_covering = set()
        # holds all the edges
        edges = list()
        ...
        while( len(edges) > 0 ):
            ...
            # we add the edges to the final list
            vertices_covering.add(edge[0])
            vertices_covering.add(edge[1])

            i = 0
            # here we remove the incident edges on either of the two verices
            while i < len(edges):
                if edges[i][0] == edge[0] or edges[i][0] == edge[1]: 
                    edges.remove(edges[i])
                elif edges[i][1] == edge[0] or edges[i][1] == edge[1]:
                    edges.remove(edges[i])
                else:
                    i+=1
